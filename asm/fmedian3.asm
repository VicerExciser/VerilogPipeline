...
; Initialize the array with numbers 17, 17+7, 17+2*7, etc.
	...
	ADDI	Zero,S1,17								; S1 is the current value of the array element for initialization  (S1 = 17)
	XOR		A1,S1,A0 									; A1 = 17 xor 0 = 17
Init:

	addi s2,s2,1		; s2 is our loop counter
	sw s2,HEX(Zero) 	; display loop count

	SW		S1,0(T0)									; Store value into an element
	ADDI	S1,S1,7								    ; Add 7 to the value for next element
	ADDI	T0,T0,4										; Move to next element (CurPtr+=4)
	BNE		T0,T1,Init								; if(CurPtr!=EndPtr) goto Init;


DebugForeverLoop:
	br DebugForeverLoop(Zero)	
